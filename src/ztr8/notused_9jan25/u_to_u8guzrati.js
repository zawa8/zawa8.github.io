const u_to_u8guzrati_dict = {
  all_phoniks_list: [
    ...
    'રi', // 	ऋ	90B	2315	vovls
    'લi', // 	ऌ	90C	2316	vovls
    'અ', // 	ऍ	90D	2317	no	candra e
    'e', // 	ऎ	90E	2318	no	short e
    'e', // 	ए	90F	2319	vovls
    'e', // 	ऐ	910	2320	vovls
    'અao', // 	ऑ	911	2321		candra o
...
export function u_to_u8guzrati(input,isa8) { //alert("in u_to_u8guzrati");
  ...
  // alert("u_to_u8guzrati_dict");
  // let u_to_u8guzrati_dict = u_to_u8guzrati_dict;
    ...
    if (curr_lang_code>0x11 && curr_lang_code<0x1B) {
      ...
      if( 0x39 === curr_char_modulo ) {
        if(
          ...
          is_in_it(u_to_u8guzrati_dict.hard_consonants_modulo_list, prev_char_modulo)
        ) {
           if(isa8) output += 'j'; else output += 'હ';
          }
        else {
          if(isa8) output += 'j'; else output += 'હ';
         }
      }
      else if(2 == curr_char_modulo){ // 'મां: ऐસ्પिરेंટ्સ મें गुરी' : 'ma: espireNts mein guri', //  ं	902 anuswara	anusvara bindu
        ...
        {
          if(isa8) output += 'q'; else output += 'અ';
         }
        output += u_to_u8guzrati_dict.all_phoniks_list[curr_char_modulo];
      }
      else { output += u_to_u8guzrati_dict.all_phoniks_list[curr_char_modulo]; }
      indeks++ ;
