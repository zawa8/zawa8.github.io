...
export function u_to_u8bngali(input,isa8) { //alert("in u_to_u8bngali");
  ...
  // alert("u_to_u8bngali_dict");
  // let u_to_u8bngali_dict = u_to_u8bngali_dict;
    ...
    if (curr_lang_code>0x11 && curr_lang_code<0x1B) {
      ...
      if( 0x39 === curr_char_modulo ) {
        if(
          ...
          is_in_it(u_to_u8bngali_dict.hard_consonants_modulo_list, prev_char_modulo)
        ) {
           if(isa8) output += 'j'; else output += 'হ';
          }
        else {
          if(isa8) output += 'j'; else output += 'হ';
         }
      }
      else if(2 == curr_char_modulo){ // 'মां: ऐস्পिরेंট्স মें गुরी' : 'ma: espireNts mein guri', //  ं	902 anuswara	anusvara bindu
        ...
        {
          if(isa8) output += 'q'; else output += 'অ';
         }
        output += u_to_u8bngali_dict.all_phoniks_list[curr_char_modulo];
      }
      else { output += u_to_u8bngali_dict.all_phoniks_list[curr_char_modulo]; }
      indeks++ ;
